        ...
        double *tension, int *num_beads, enum SimType *sim_type)
{
    ...
    if (*sim_type == STRING)
        fscanf(fp, "%lf", tension);

    fscanf(fp, "%d", num_beads);
    
    /* Allocate memory for beads and connections */
    *beads = calloc(*num_beads, sizeof(Bead));
    if (*beads == NULL)
    {
    ...
    *connections = calloc(*num_beads + 1, sizeof(double));
    if (*connections == NULL)
    {
    ...
    {
        fscanf(fp, "%lf", &(*connections)[i]);
        fscanf(fp, "%lf %lf %lf", &((*beads)[i].mass),
                &((*beads + i)->x0),
                &((*beads + i)->v0));
    }

    /* There's one more connection than bead; scan that in */
    fscanf(fp, "%lf", &(*connections)[*num_beads]);
